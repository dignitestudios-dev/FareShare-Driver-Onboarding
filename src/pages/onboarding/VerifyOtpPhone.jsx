  ...
  const { values, handleBlur, handleChange, handleSubmit, errors, touched } =
    useFormik({
      ...
      onSubmit: async (values, action) => {
        ...
        console.log(values);
        try {
          const response = await axios.post(`/auth/validatePhoneOTP`, {
      ...
      axios
        ...
        .then((response) => {
          ...
          if (response?.status === 201) {
            setResend(false);
            SuccessToast("Phone OTP Resent Sent Successfully.");
          }
        });
    } catch (error) {
      console.log(error);
      ...
      <div class="grid lg:grid-cols-4 md:grid-cols-3 items-center">
        ...
        <div class="flex flex-col justify-center space-y-16 md:h-screen max-md:mt-16 px-4 min-h-full bg lg:px-8  ">
          <div className="px-4  p-4">
            <h4 class="text-white text-lg font-semibold">Verify Email OTP</h4>
          ...
          <div className="px-4  rounded-2xl p-4 bg-white">
